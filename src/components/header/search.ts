...
function getCategoriesWithDepth(
...
export default class Search extends Vue {
	...
	@Inject({ from: "getSearchScope", default: null }) readonly getSearchScope!:
		| (() => Element)
		| null;
	...
	async mounted() {
		...
		this.categories = getCategoriesWithDepth(
			await this.$shopApi.getCategories({ depth: 1 })
		);
	}
	...
	async search() {
		...
		const products =
			query === ""
				? []
				: await this.$shopApi.getSuggestions(query, {
