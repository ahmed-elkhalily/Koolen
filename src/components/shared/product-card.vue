<template>
	...
	>
		<AsyncAction
			...
			:action="
				() => $store.dispatch('quickview/open', { productSlug: product.slug })
			"
		>
			<button
				type="button"
		...
		<div class="product-card__info">
			...
			<ul v-if="features.length" class="product-card__features-list">
		...
		<div class="product-card__actions">
			...
			<div class="product-card__buttons">
				...
				<AsyncAction
					v-slot="{ run, isLoading }"
					:action="() => $store.dispatch('compare/add', { product })"
				>
					<button
						type="button"
						:class="[
