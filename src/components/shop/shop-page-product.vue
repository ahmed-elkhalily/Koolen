	...
	<div>
		...
		<div v-if="layout === 'sidebar'" class="container">
			<div :class="`shop-layout shop-layout--sidebar--${sidebarPosition}`">
				<ProductSidebar
					v-if="sidebarPosition === 'start'"
					class="shop-layout__sidebar"
				/>
...
<script lang="ts">
	...
	export default class ShopPageProduct extends Vue {
		...
		readonly layout!: ShopPageProductLayout;
		@Prop({ type: String, default: () => "start" })
		readonly sidebarPosition!: ShopPageProductSidebarPosition;
		...
		mounted() {
			this.$shopApi
				.getRelatedProducts(this.product.slug, { limit: 8 })
				.then((products) => {
