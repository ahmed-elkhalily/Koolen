	...
	>
		...
		<div v-if="productsList.items.length !== 0" class="products-view__content">
				...
				>
					<div class="view-options__filters-button">
						...
						>
							...
							<span v-if="filtersCount" class="filters-button__counter">{{
								filtersCount
							}}</span>
						</button>
					</div>
					...
					<div class="view-options__legend">
						Showing {{ productsList.from }}—{{ productsList.to }} of
			...
			<div
				...
				:data-with-features="
					currentLayout === 'grid-with-features' ? 'true' : 'false'
				"
				:data-mobile-grid-columns="2"
			>
				<div class="products-list__body">
					<div
						v-for="product in productsList.items"
						:key="product.id"
...
<script lang="ts">
	...
	// components
	import NewProduct from "~/components/shared/new/new-product.vue";

	export type ProductsViewLayout = "grid" | "grid-with-features" | "list";
	export type ProductsViewGrid =
		| "grid-3-sidebar"
		| "grid-4-full"
		| "grid-5-full";
	...
	@Component({
		components: { Pagination, ProductCard, Filters16Svg, NewProduct },
	})
	export default class ProductsView extends Vue {
		@Prop({ type: String, default: () => "grid" })
		readonly layout!: ProductsViewLayout;
		@Prop({ type: String, default: () => "grid-3-sidebar" })
		readonly grid!: ProductsViewGrid;
		@Prop({ type: String, default: () => "mobile" })
		readonly offcanvas!: ProductsViewOffcanvas;
